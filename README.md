...
### Почему EXE может «не запускаться»

Если вы собрали проект с флагом `-DCLEANAI_HEADLESS_CI=ON`, вы получите **не полноценное приложение**, а CI-заглушку.
Она не открывает главное окно CleanAI, поэтому при двойном клике может казаться, что ничего не произошло.

Теперь такая сборка показывает понятное сообщение с подсказкой. Для обычного приложения собирайте проект **без** `CLEANAI_HEADLESS_CI=ON`.
