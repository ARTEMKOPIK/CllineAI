...
Скачивайте для пользователей только `CleanAI-win64.zip`, распакуйте архив полностью и запустите `CleanAI.exe`. Не берите `CleanAI-headless-exe` — это техническая заглушка для диагностики CI. И не запускайте `CleanAI.CoreLogicTests.exe`: это служебная проверка, после неё окно закрывается — так и задумано.
